...
import human.dto.KategorieDTO;
import human.dto.SearchDTO;
import human.util.DBConnection;

public class Board_GDAO extends DBConnection {
	...
	public int insert(BoardDTO dto) {
		...
		try {
			if(dto.getOriginfile_name() != null) {
				...
				String sql = "insert into tb_board_G (b_idx, m_idx, writer, title, content, originfile_name, savefile_name, kategorie) "
				...
				pstmt.setString(7, dto.getKategorie());
				
			}else {
				//파일업로드가 이루어지지 않은 경우의 insert문
				String sql = "insert into tb_board_G (b_idx, m_idx, writer, title, content, kategorie) "
						     + " values(board_seq.nextval, ?,?,?,?,?)";
				...
				pstmt.setString(3, dto.getTitle());
				pstmt.setString(4, dto.getContent());
				pstmt.setString(5, dto.getKategorie());
			}
	...
	public ArrayList<BoardDTO> getBoards(){
		...
		String sql = "select * from tb_board_G "
				+ "where del_or_not = 1 "
				+ "order by b_idx desc";
	...
	//카테고리가 있을때?
	public ArrayList<BoardDTO> getBoards(SearchDTO sDto, KategorieDTO kDto){
		ArrayList<BoardDTO> boardList = new ArrayList<BoardDTO>();

		try {
			//검색어가 입력되었는지 여부에 따라 SQL문을 구분함
			String sql = null;
			if(sDto.getSearchWord() != null && kDto.getType() != null) {//검색어가 입력된 경우
				//검색영역에 따라 구분을 나눔:switch
				String searchField = null;
				String type = null;
				switch(sDto.getSearchField()) {
	...
	public void updateRead_cnt(int b_idx) {
		...
		String sql = "update tb_board_G set read_cnt = (read_cnt + 1) where b_idx =?";
		try {
			pstmt = conn.prepareStatement(sql);
	...
	public BoardDTO getBoard(int b_idx){
		...
		String sql = "select * from tb_board_G where b_idx =?";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, b_idx);
			rs = pstmt.executeQuery();//조회된 결과를 ResultSet객체에 담음
	...
	public int update(BoardDTO dto) {
		...
		try {
			if(dto.getOriginfile_name() != null) {
				//파일업로드가 이루어질 경우의 update문
				String sql = "update tb_board_G set title=?, content=?, originfile_name=?, savefile_name=?"
						     + " where b_idx = ?";
				...
				//파일업로드가 이루어지지 않은 경우의 update문
				String sql = "update tb_board_G set title=?, content=? "
					     + " where b_idx = ?";
	...
	public int delete(int b_idx) {
		...
		try {
			...
			String sql = "update tb_board_G set del_or_not=-1"
					     + " where b_idx = ?";
